...
public extension LocationManager {
    ...
    ///     )
    ///
    static let live: LocationManager = { () -> LocationManager in
...
private class LocationManagerDelegate: NSObject, CLLocationManagerDelegate {
    ...
    func locationManager(
        _: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus
    ) {
    ...
    func locationManager(_: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
    ...
    #if os(macOS)
        func locationManager(
            _: CLLocationManager, didUpdateTo newLocation: CLLocation,
        ...
        func locationManager(
            _: CLLocationManager, didFinishDeferredUpdatesWithError error: Error?
        ) {
    ...
    #if os(iOS) || os(macOS) || targetEnvironment(macCatalyst)
        func locationManager(
            _: CLLocationManager, didDetermineState state: CLRegionState, for region: CLRegion
        ) {
    ...
    #if os(iOS) || os(macOS) || targetEnvironment(macCatalyst)
        func locationManager(
            _: CLLocationManager, monitoringDidFailFor region: CLRegion?, withError error: Error
        ) {
            subscriber.send(
                .monitoringDidFail(
    ...
    #if os(iOS) || targetEnvironment(macCatalyst)
        func locationManager(
            _: CLLocationManager, didRange beacons: [CLBeacon],
    ...
    #if os(iOS) || targetEnvironment(macCatalyst)
        func locationManager(
            _: CLLocationManager, didFailRangingFor beaconConstraint: CLBeaconIdentityConstraint,
            error: Error
        ) {
            subscriber.send(
